...
public class ChaosMonkeyForNodesEvent extends OrchestrationSelfScheduledExperimentAction {
     ...
     * @param showInTrace boolean: Declaration if this event should be shown in the
     *                    trace
     * @param namePattern String: A regex: if node name matches regex it is
     *                    considered as a candidate for failure
     * @param instances   int: The number of instances of the specified deployment
     *                    you want to shut down, can be
    ...
    public void eventRoutine() throws SuspendExecution {
        ...
        List<Node> candidates = nodes.stream().filter(n -> pattern.matcher(n.getPlainName()).find())
                .collect(Collectors.toList());
        List<Node> nodesToKill = new ArrayList<>(instances);
        if (candidates.size() < instances) {
            sendTraceNote("Could not execute ChaosMonkeyForNodesEvent because only " + candidates.size()
                    + " candidates available, " + instances + " were required");
            return;
        ...
        List<Pod> failedPods = new ArrayList<>(instances);
        for (Node n : nodesToKill) {
            List<Pod> pods = n.failInstantly();
            failedPods.addAll(pods);
        ...
        for (Scheduler s : ManagementPlane.getInstance().getActiveSchedulers()) {
            s.onNodeFailure(nodesToKill, failedPods);
        }
    }
